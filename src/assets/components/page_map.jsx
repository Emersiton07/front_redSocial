...
function App() {
  ...
  const getRoute = async () => {
    ...
    const coordinates = selectedObras.map((obra) => `${obra.ubicacion.longitud},${obra.ubicacion.latitud}`).join(";");

    const url = `https://api.mapbox.com/directions/v5/mapbox/walking/${coordinates}?geometries=geojson&access_token=pk.eyJ1IjoiZW1lcnNvbjA3IiwiYSI6ImNtNmNtM3IxczBrc3gyanE0aGN1eTlmOWYifQ.jw_Yp6JZCKeMeq0zEDzvmg`;
  ...
  return (
    ...
    <Modal title="Calificar publicación" open={isModalOpen} onOk={handleOk} onCancel={handleCancel}>
      <Rate onChange={(value) => setRating(value)} value={rating} />
        ...
        {isRoutePanelOpen && (
          <div className="absolute top-25 right-40 bg-white p-4 shadow-lg rounded-lg w-64 max-h-96 overflow-auto">
            <h3 className="text-lg font-semibold">Seleccionar Obras</h3>
            {obras.map((obra) => (
              <div key={obra._id} className="flex items-center space-x-2 text-blue-500">
                <input
                  type="checkbox"
